...
public Action Command_Marker(int client, int args)
{
	if (warden_iswarden(client))
	{
		...
		menu.SetTitle("[ByDexter] Marker - Properties ?\n ");
		if (g_MarkerSize == 100.0)
			menu.AddItem("Buyult", "Size: Small");
		else if (g_MarkerSize == 150.0)
			menu.AddItem("Buyult", "Size: Medium");
		else if (g_MarkerSize == 200.0)
			menu.AddItem("Buyult", "Size: Large");
		else if (g_MarkerSize == 250.0)
			menu.AddItem("Buyult", "Size: XLarge");
		else if (g_MarkerSize == 300.0)
			menu.AddItem("Buyult", "Size: XXLarge");
		if (g_MarkerColorStat == 0)
			menu.AddItem("RazerYap", "Color: Blue");
		else if (g_MarkerColorStat == 1)
			menu.AddItem("RazerYap", "Color: Red");
		else if (g_MarkerColorStat == 2)
			menu.AddItem("RazerYap", "Color: Green");
		else if (g_MarkerColorStat == 3)
			menu.AddItem("RazerYap", "Color: Yellow");
		else if (g_MarkerColorStat == 4)
			menu.AddItem("RazerYap", "Color: White");
		else if (g_MarkerColorStat == 5)
			menu.AddItem("RazerYap", "Color: Pink");
		else if (g_MarkerColorStat == 6)
			menu.AddItem("RazerYap", "Color: Rainbow");
		if (g_MarkerWidht == 3.0)
			menu.AddItem("Kalinlastir", "Widht: 3.0");
		else if (g_MarkerWidht == 5.0)
			menu.AddItem("Kalinlastir", "Widht: 5.0");
		else if (g_MarkerWidht == 8.0)
			menu.AddItem("Kalinlastir", "Widht: 8.0");
		else if (g_MarkerWidht == 10.0)
			menu.AddItem("Kalinlastir", "Widht: 10.0");
		if (g_MarkerType == 0)
		{
			if (g_MarkerSpeed == 0)
				menu.AddItem("Akisyukselt", "Speed: Off");
			else if (g_MarkerSpeed == 10)
				menu.AddItem("Akisyukselt", "Speed: Slow");
			else if (g_MarkerSpeed == 35)
				menu.AddItem("Akisyukselt", "Speed: Normal");
			else if (g_MarkerSpeed == 50)
				menu.AddItem("Akisyukselt", "Speed: Fast");
			else if (g_MarkerSpeed == 100)
				menu.AddItem("Akisyukselt", "Speed: High Fast");
		}
		else if (g_MarkerType == 1)
			menu.AddItem("Akisyukselt", "Speed: Closed", ITEMDRAW_DISABLED);
		if (g_MarkerAmplitude == 0.0)
			menu.AddItem("Titresimyukselt", "Amplitude: Off");
		else if (g_MarkerAmplitude == 1.0)
			menu.AddItem("Titresimyukselt", "Amplitude: Slow");
		else if (g_MarkerAmplitude == 3.0)
			menu.AddItem("Titresimyukselt", "Amplitude: Normal");
		else if (g_MarkerAmplitude == 5.0)
			menu.AddItem("Titresimyukselt", "Amplitude: Fast");
		else if (g_MarkerAmplitude == 10.0)
			menu.AddItem("Titresimyukselt", "Amplitude: High Fast");
		if (g_MarkerType == 0)
			menu.AddItem("Gorunumdegis", "Type: Beam");
		else if (g_MarkerType == 1)
			menu.AddItem("Gorunumdegis", "Type: Smooth");
		menu.Display(client, MENU_TIME_FOREVER);
		return Plugin_Handled;
	}
	else
	{
		CReplyToCommand(client, "{green}[ByDexter] {default}This command can only be {blue}used by the warden!");
		return Plugin_Handled;
	}
}
...
public int Menu_CallBack(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			CReplyToCommand(param1, "{green}[ByDexter] {default}Settings are not made because {blue}you are dead!");
			delete menu;
		}
	}
